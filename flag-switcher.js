...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (currentFlag && flagDropdown) {
        ...
        // 点击开关：避免 hover 时还没移动过去就收起
        currentFlag.addEventListener('click', function(e){
            e.stopPropagation();
            flagDropdown.classList.toggle('open');
        });
        // 点击下拉内部不会冒泡到 document
        flagDropdown.addEventListener('click', function(e){ e.stopPropagation(); });
        // 点击页面其他位置关闭
        document.addEventListener('click', function(){ flagDropdown.classList.remove('open'); });
